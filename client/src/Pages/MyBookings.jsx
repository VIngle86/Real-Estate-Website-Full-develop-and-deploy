...
import { dummyBookingsData, dummyProperties } from '../assets/data'
...
const MyBookings = () => {
  ...
  return (
    <div className='max-padd-container bg-fradient-to-r from-[#fffbee] to-white py-16 pt-28'>
      {bookings?.map((booking) => (
        <div key={booking._id} className='bg-white ring-1 ring-slate-900/5 p-2 pr-4 mt-3 rounded-lg'>
          ...
          <div className='flex flex-col lg:flex-row justify-between items-start lg:item-center gap-3 border-t border-gray-300 pt-3'>
            ...
            <div className='flex gap-2 gap-x-3'>
              ...
              {!booking.isPaid && (<button className='btn-secondary py-1 text-xs rounded-sm'>Pay Now</button>)}
            </div>
          </div>
        </div>
      ))}
