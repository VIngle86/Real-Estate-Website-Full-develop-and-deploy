...
const Dashboard = () => {
  ...
  return (
    <div className='md:px-8 py-6 xl:py-8 m-1 sm:m-3 h-[97vh] over-flow-y-scroll lg:w-11/12 bg-white shadow rounded-xl'>
          ...
          {dashboardData.bookings.map((booking, index) => (
            <div key={index} className='flex justify-between items-center flex-wrap gap-2 sm:grid grid-cols-[2fr_2fr_1fr_1fr]  lg:grid-cols-[0.5fr_2fr_2fr_1fr_1fr] px-6 py-3 bg-secondary/5 text-gray-50 medium-14 border-b-1 border-slate-900/15'>
              ...
              <div className='flexStart gap-x-2 max-w-64'>
                <div className='overflow-hidden rounded-lg'><img src={booking.property.images[0]} alt={booking.property.title} className='w-16 rounded-lg' /></div>
              ...
              <button className={`${booking.isPaid ? "bg-green-400/80 text-white" : "bg-secondary/10 text-red-500"} w-22 py-0.5 rounded-full text-xs border border-greeen-500/30`}>{booking.isPaid ? "Completed" : "Pending"}</button>
