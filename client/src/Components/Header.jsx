  ...
  useEffect(() => {
    const handleScroll = () => {
      if (location.pathname === '/') {
        ...
        setActive(true)//always stay active on other pages like contact page
      )
      ...
      <div className='max-padd-container'>
        ...
        <div className='flexBetween'>
          ...
          <div className='flex flex-1'>
            <Link to={'/'}><img src={assets.logoImg} alt="logoImg" className={`${!active && "invert"} h-11`} /> </Link>
          </div>
          {/*Navbar*/}
          <Navbar
            setMenuOpened={setMenuOpened}
            containerStyles={`${menuOpened ? "flex items-start flex-col gap-y-8 fixed top-16 right-6 p-5 bg-white shadow-md w-52 ring-1 ring-slate-900/5 rounded-x1 z-50" : "hidden lg:flex gap-x-5 xl:gap-x-l medium-15 p-1"} ${!menuOpened && !active ? "text-white" : ""}`}
          />
          ...
          <div className='flex sm:flex-1 items-center sm:justify-end gap-x-4 sm:gap-x-8'>
            ...
            {/* Menu Toggle */}
            <>
              {menuOpened ? (<img src={assets.close} alt="closeMenuIcon" onClick={toggleMenu} className={`${!active && "invert"} lg:hidden cursor-pointer text-xl`} />
              ) : (<img src={assets.menu} alt="MenuIcon" onClick={toggleMenu} className={`${!active && "invert"} lg:hidden cursor-pointer text-xl`} />
              )}
