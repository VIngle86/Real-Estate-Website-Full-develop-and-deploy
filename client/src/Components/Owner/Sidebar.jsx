...
const Sidebar = () => {
  ...
  return (
    <div className='bg-gradient-to-r from-[#fffbee] to-white sticky'>
      <div className='mx-auto max-w-[1440px] flex flex-col md:flex-row '>
        ...
        <div className='max-md:flexCenter flex flex-col justify-between bg-white sm:m-3 md:min-w-[20%] md:min-h-[97vh] rounded-xl shadow'>
          <div className='flex flex-col gap-y-6 max-md:items-center md:flex-col md:pt-5'>
            ...
            <div className='flex md:flex-col md:gap-x-5 gap-y-8 md:mt-4'>
              {navItems.map((link) => (
                ...
                >
                  <img src={link.icon} alt={link.label} className="hidden md:flex" width={18} />
                  <div>
                    {link.label}
                  </div>
                </NavLink>
          ...
          <div className='hidden md:flex items-center gap-3 md:bg-primary border-t border-slate-900/15 rouned-b-xl p-2 pl-5 lg:pl-10 md:mt-10'>
            <UserButton appearance={{
              elements: {
                userButtonAvatarBox: {
                  width: "45px",
